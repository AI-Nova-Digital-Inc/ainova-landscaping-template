...
const serviceTypes = ["Weekly Mowing & Edging","Bi-Weekly Mowing","Spring Cleanup","Fall Cleanup","Full Season Program","One-Time Service","Other"];
export default function LawnForm() {
  const [sent, setSent] = useState(false);
  return (
    <section id="estimate" className="relative overflow-hidden px-4 py-16 lg:py-24">
      <Image src="/images/lawn-maintenance/hero/hero-bg.jpg" alt="Lawn background" fill className="object-cover object-center" />
      ...
      <div className="relative z-10 mx-auto max-w-2xl">
        <div className="rounded-3xl border border-white/12 bg-white/10 p-8 shadow-luxury backdrop-blur-md sm:p-10">
          ...
          <h2 className="font-serif text-2xl font-black text-warm-white sm:text-3xl">Get A Lawn Maintenance Quote</h2>
          ...
          {sent ? (
            ...
            <form onSubmit={(e) => { e.preventDefault(); setSent(true); }} className="mt-8 grid gap-4 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2">
                <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-warm-white/70">Service Type</label>
              ...
              <div className="sm:col-span-2">
                <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-warm-white/70">Tell Us About Your Property</label>
                <textarea rows={4} placeholder="Describe your lawn size, current condition, any special requirements…" className="w-full resize-none rounded-xl border border-white/20 bg-white/10 px-4 py-3 text-sm text-warm-white placeholder-warm-white/40 backdrop-blur-sm transition focus:border-gold/50 focus:outline-none focus:ring-1 focus:ring-gold/30" />
              </div>
              <div className="sm:col-span-2">
                <button type="submit" className="w-full rounded-xl bg-gold px-6 py-4 font-black text-charcoal shadow-luxury transition hover:-translate-y-0.5 hover:bg-yellow-400">Get A Free Quote</button>
              </div>
            </form>
          )}
        </div>
