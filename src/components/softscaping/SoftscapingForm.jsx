...
const projectTypes = [
  ...
  "Full Softscaping Design",
  "Other",
];

export default function SoftscapingForm() {
  const [sent, setSent] = useState(false);

  return (
    <section id="estimate" className="relative overflow-hidden px-4 py-16 lg:py-24">
      <Image
        src="/images/softscaping/softscaping-form-background.png"
          ...
          {sent ? (
            ...
            <form onSubmit={(e) => { e.preventDefault(); setSent(true); }} className="mt-8 grid gap-4 sm:grid-cols-2">
              {[
                ...
                { label: "Phone Number", type: "tel", placeholder: "+1 (519) 000-0000", required: false },
                { label: "Address or Street", type: "text", placeholder: "Property address", required: false },
                ...
                <div key={label}>
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-white/20 bg-white/10 px-4 py-3 text-sm text-warm-white placeholder-warm-white/40 backdrop-blur-sm transition focus:border-gold/50 focus:outline-none focus:ring-1 focus:ring-gold/30"
                  />
                </div>
              ))}
              ...
              <div className="sm:col-span-2">
                ...
                <textarea
                  rows={4}
                  placeholder="Describe your garden goals, existing plants, timeline…"
                  className="w-full resize-none rounded-xl border border-white/20 bg-white/10 px-4 py-3 text-sm text-warm-white placeholder-warm-white/40 backdrop-blur-sm transition focus:border-gold/50 focus:outline-none focus:ring-1 focus:ring-gold/30"
                />
              </div>
              ...
              <div className="sm:col-span-2">
                <button
                  type="submit"
                  className="w-full rounded-xl bg-gold px-6 py-4 font-black text-charcoal shadow-luxury transition hover:-translate-y-0.5 hover:bg-yellow-400"
                >
                  Get A Free Estimate
                </button>
              </div>
